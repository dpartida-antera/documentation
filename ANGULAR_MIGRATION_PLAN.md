...
This is delivered in two stages so there is value immediately and a clear upgrade path.

**Local snapshot history (already added to the current static QA pages, carries over to Angular):** each QA page has a tester-name field and a "Save this run" button. Saving snapshots the current statuses and notes into a timestamped entry kept in localStorage (`antera_po_qa_history_v1`, `antera_aa_qa_history_v1`). A "Saved runs" panel lists each run with its date, tester, and pass/fail/blocked tally, and lets you re-open it into the runner, export it as CSV or JSON, or delete it. This is per browser/machine, so it is durable for one user but not shared. In Angular this becomes the `QaStateService` plus a `RunHistoryComponent`, unchanged in behavior.

**Shared run history (Phase 6, backend):** to share runs across testers and devices, the snapshots move to a backend. The data model is small and maps directly onto the snapshot already produced today:

| Table | Key fields |
|---|---|
| `qa_run` | `id`, `suite` (shipping or allocation), `tester`, `saved_at`, optional `build_label`, summary counts |
| `qa_result` | `run_id` (FK), `scenario_id`, `status`, `notes` |

The same `RunHistoryComponent` would read from `ApiService` instead of localStorage, so the UI does not change. Authentication identifies the tester (SSO if embedded in the Antera product, Supabase or Firebase auth if standalone). A migration path lets a user push their existing local snapshots to the backend on first sign-in.

**Backend choice for shared runs:**

| Scenario | Recommended backend |
|---|---|
| Embedded in the Antera product | Add `qa_run` and `qa_result` endpoints to Antera's existing backend and database; reuse its login/SSO. No new infrastructure. |
| Standalone docs site | Managed BaaS: Supabase (Postgres, auth, row-level security) recommended; Firebase as an alternative. |
| Full control preferred | A thin Node or .NET API plus Postgres on Antera's existing cloud. |
